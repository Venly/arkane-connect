...
export class BuildSimpleTransactionRequest extends BuildTransferRequestBase implements BuildTransferRequestDto {
    ...
    public static fromData(requestData: BuildTransferRequestDto): BuildSimpleTransactionRequest {
        const {walletId, to, secretType, value, data, network} = requestData;
        return new this(walletId, to, secretType, value, data, network);
    }

    public value?: number;
    public data?: string;

    constructor(walletId: string, to: string, secretType: SecretType, value?: number, data?: string, network?: Network) {
        super(BuildTransactionRequestType.TRANSFER, walletId, to, secretType, network);
        this.value = value;
